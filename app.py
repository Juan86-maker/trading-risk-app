        ...
        if submitted:
            ...
            if lote is None or precio is None:
                ...
                datos = {
                    ...
                    "Símbolo": symbol,
                    "Tipo": side,
                    "Lote": f"{lote:.2f}" if lote is not None else "",
                    "Precio": f"{precio:.2f}" if precio is not None else "",
                    "Stop Loss": f"{sl:.2f}" if sl is not None else "",
                    "Take Profit": f"{tp:.2f}" if tp is not None else "",
                    "Margen": f"{margen:.2f}" if margen is not None else "",
                    "Riesgo": f"{riesgo:.2f}" if riesgo is not None else "",
                    "Beneficio": f"{beneficio:.2f}" if beneficio is not None else "",
